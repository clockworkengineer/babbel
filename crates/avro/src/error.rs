    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::UnexpectedEof { expected, available } => {
                write!(f, "unexpected end of Avro input: expected {} bytes, available {}", expected, available)
            }
            ...
            Self::SyncMarkerMismatch => write!(f, "Avro OCF 16-byte sync marker mismatch between blocks"),
...
impl From<AvroError> for BabbelError {
    fn from(err: AvroError) -> Self {
        match &err {
            ...
            AvroError::InvalidUtf8 => {
                BabbelError::encoding(err.to_string()).with_format("avro")
            }
