...
pub use stringify::{PrettyOptions, emit_pretty_to, emit_to};
...
pub fn from_slice(bytes: &[u8]) -> Result<Node, TomlError> {
    let s =
        core::str::from_utf8(bytes).map_err(|_| TomlError::custom("Input is not valid UTF-8"))?;
    from_str(s)
}
...
"#;
        ...
        assert_eq!(
            node.get("title").and_then(|n| n.as_str()),
            Some("TOML Example")
        );

        let owner = node.get("owner").expect("Owner table missing");
        assert_eq!(
            owner.get("name").and_then(|n| n.as_str()),
...
"#;
        ...
        let products = node
            .get("products")
            .and_then(|n| n.as_array())
            .expect("Products array missing");
        assert_eq!(products.len(), 2);
        assert_eq!(
            products[0].get("name").and_then(|n| n.as_str()),
            Some("Hammer")
        );
        assert_eq!(
            products[1].get("name").and_then(|n| n.as_str()),
            Some("Nail")
        );
    }
