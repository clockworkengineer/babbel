...
fn test_xml_source_slice_range_safe_on_invalid_boundaries() {
    ...
    let source = babbel_xml::XmlSource::from_string("€uro");
...
fn test_streaming_io_limit_enforced() {
    ...
    let res = babbel_xml::XmlSource::from_reader_with_limit(stream, 1024);
    assert!(res.is_err());
    match res.unwrap_err() {
        XmlError::SecurityLimitExceeded(msg) => {
    ...
    let res_ok = babbel_xml::XmlSource::from_reader_with_limit(valid_stream, 1024);
    assert!(res_ok.is_ok());
}
...
fn test_xpath_parser_depth_limit() {
    ...
    let mut parser = babbel_xml::xpath::parser::XPathParser::new(&expr).unwrap();
