...
use babbel_json::{
    BufferDestination, BufferSource, FileDestination, FileSource, Format, Node, detect_format,
    parse, read_file_to_string, stringify, write_file_from_string,
};
...
fn main() {
    ...
    if Path::new(unicode_test_file).exists() {
        match FileSource::new(unicode_test_file) {
            Ok(mut source) => match parse(&mut source) {
                Ok(node) => {
    ...
    let cleanup_files = vec!["converted_utf16.json", "unicode_test.json"];
