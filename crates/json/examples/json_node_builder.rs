...
fn main() {
    ...
    println!("\n2. Building arrays:");
    let simple_array = Node::Array(vec![Node::from(1), Node::from(2), Node::from(3)]);
    ...
    println!(
        "  Simple array length: {}",
        simple_array.as_array().unwrap().len()
    );
    ...
    println!(
        "  Employee structure created with {} top-level fields",
    ...
    if let Some(city) = employee_node
        .get("address")
    ...
    if let Some(first_skill) = employee_node.get("skills").and_then(|s| s.at(0)) {
    ...
    let mut numbers = Node::Array(vec![Node::from(1), Node::from(2), Node::from(3)]);
    ...
    let nums = Node::Object({
        ...
        map.insert(
            "i64".to_string(),
            Node::Number(Numeric::Integer(9223372036854775807)),
        );
        ...
        map.insert(
            "u64".to_string(),
            Node::Number(Numeric::UInteger(18446744073709551615)),
        );
    ...
    println!(
        "  Created object with {} numeric types",
        nums.as_object().unwrap().len()
    );
