...
use babbel_json::{BufferSource, Node, parse};
  ...
  "catalog": {
    ...
    for product in &all_products {
        if let Some(tags) = product.get("tags").and_then(|t| t.as_array()) {
            let has_gaming_tag = tags.iter().any(|tag| tag.as_str() == Some("gaming"));
