...
use babbel_json::{BufferDestination, BufferSource, Node, parse, stringify};
...
fn main() {
    ...
    let node2 = Node::Object({
        ...
        map.insert(
            "items".to_string(),
            Node::Array(vec![Node::from(10), Node::from(20)]),
        );
        map
    });
    ...
    println!(
        "  Layer 3 (runtime): {}",
        node_to_string(&runtime_overrides)
    );
